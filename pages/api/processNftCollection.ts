  ...
  for (let i = 1; i <= parseInt(metadata.totalSupply); i++) {
    ...
    if (batchedPromises.length >= batchSize) {
      ...
      const cleanResults = results
        ...
        .map((r) => {
          stats.trades =
            stats.trades + r.activity.filter((a) => a.event === "Sale").length;
          ...
          stats.volume = BigNumber.from(stats.volume).add(
            getSalesVolume(r.activity)
          );
          // the first transaction is the one representing the sale
          stats.washVolume = BigNumber.from(stats.washVolume).add(
            getSalesVolume(r.washTrades.map((trade) => trade.transactions[0]))
          );
